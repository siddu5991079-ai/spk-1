...
// 🛠️ AUTOMATIC IFRAME PARSER (Yeh URL nikal lega)
let rawInput = process.env.TARGET_URL || 'https://dadocric.st/player.php?id=starsp3&v=m';
let extractedUrl = rawInput;
if (rawInput.includes('<iframe') && rawInput.includes('src=')) {
    const match = rawInput.match(/src="([^"]+)"/);
    if (match && match[1]) {
        extractedUrl = match[1];
        console.log(`[+] Iframe detected! Extracted URL: ${extractedUrl}`);
    }
}

const TARGET_URL = extractedUrl;
...
async function startDirectStreaming() {
    ...
    browser = await puppeteer.launch({
        headless: false, // Xvfb will handle this in background
...
// =========================================================================
// ⏱️ AUTO-OVERLAP TRIGGER (Using Native GitHub CLI)
// =========================================================================
setTimeout(() => {
    console.log("\n[*] 5h 50m completed! Triggering next action for seamless overlap...");
    try {
        const { execSync } = require('child_process');
        // rawInput use kar rahe hain taake agle run ko bhi wahi data mile
        const cmd = `gh workflow run main.yml -f target_url="${rawInput}" -f okru_stream_channel="${SELECTED_CHANNEL}" -f stream_quality="${process.env.STREAM_QUALITY}"`;
